...
1. ユーザーが指示に用いた語
2. プロジェクトの用語集
3. リポジトリの識別子、ドキュメントにある語

- 上位の語は、後述の禁止語彙にあってもそのまま使う。
- 同義語への言い換え、外来語と漢語の相互変換（テキスト→文言 など）をしない。
- 文体規範（敬体、産業日本語）は語の選択を上書きしない。
- 用語集とユーザーの語が衝突する場合は置き換えず確認する。
- 用語が不適切に見えても黙って置き換えず、指摘したうえで元の語を使う。

## 自分で語を作らない

- ものを指す語は、リポジトリの識別子、ドキュメント、ユーザーの発言のいずれかにある語を使う。
- そこに無い語（コードの語彙や一般名詞を組み合わせた複合語）を作らない。該当する語が無いときは、識別子を `` で示し、何を指すかを一文で説明する。
- 複数の意味を持つ語（スレッド、セッション、ジョブ、イベント、メッセージ、通知 など）は、初出で何のそれかを限定する（Slack のスレッド、OS のスレッド）。
- 点検: 提案や回答に出てくる名詞は、リポジトリで grep して見つかるか。見つからなければ作った語である。
...
| 作業動詞 | 潰す、踏み込む、塞ぐ、溶かす、逃がす、倒す、叩く、投げる、引く、落ちる | 修正する、調べる、防ぐ、費やす、呼び出す、送信する、取得する、失敗する |
| 判断 | 既定、別物、定番、要点、素朴、定石、桁違い、正本 | デフォルト、異なる、一般的、原本 |
| 定型句 | 重要なのは、ポイントは、と言えるでしょう、のではないでしょうか、結論から言うと、結論として、まとめると、〜に他ならない、正面から〜、〜を深掘りする、〜について見ていく、言うまでもなく、ぜひ〜してください | 主張をそのまま書く。予告と総括を置かない |
| 空虚な形容 | 不可欠、核心的、鍵となる、根本的な、多角的、包括的、総合的、非常に、極めて | 根拠や視点そのものを書く |
| 共感の演出 | そのお気持ち、よくわかります／大変でしたね／〜なんですよね | 書かない |

#### 許容される語彙
以下は一般的に忌避されますが、技術者としては自然なため許容されます。
- 切り分ける
- (ブランチを)切る
- 一方で
...
1. 「効く」「瞬間」「壊れる」「既定」を検索する。
2. ダッシュ、""、絵文字、太字の数を確認する。
3. 文頭の「重要なのは」「まとめると」、文末の「と言えるでしょう」を検索する。
