...
const BLOCKED_PATTERNS: &[BlockedPattern] = &[
    BlockedPattern {
        prefix: "git push",
        reason: "git push is blocked: remote push must be done manually",
    },
    BlockedPattern {
        prefix: "git reset",
...
fn check_blocked_command(command: &str) -> Option<String> {
...
pub fn handle_pre_tool_use(input: &HookInput) -> Result<()> {
    ...
    if tool_name == "Bash" {
        if let Some(cmd) = command {
            if let Some(reasons) = check_blocked_command(cmd) {
                return Err(anyhow::anyhow!("{}", reasons));
            }
        }
    }
        ...
        if is_file_writing_tool {
            ...
            if ext == "sh" && !allow_all && !allowed.contains(&"sh") {
                return Err(anyhow::anyhow!(
                    "Creating or editing ad hoc .sh files is prohibited"
                ));
            }
            ...
            if ext == "md" && !allow_all && !allowed.contains(&"md") {
                return Err(anyhow::anyhow!(
                    "Creating or editing .md document files is prohibited"
                ));
            }
        }
    }
