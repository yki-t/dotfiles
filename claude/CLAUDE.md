...
- ユーザーが、自分の提案するA案を選択したので、そのまま実装を開始した。
    - 正しい対応
    - ユーザーがコメントなく提案を採用した場合、承認を得たと見なして実装を開始して問題ありません。
...
- ユーザーが、自分の提案を採用したが、実装方法の詳細について未提案だったので改めて提案した。
    - 誤った対応
    - ユーザーが特に何もコメントせず提案を採用した場合、承認を得たと見なして実装を開始して問題ありません。詳細な実装方法(例えばどのファイルのどこを変更するとか、コードを示した差分)については、ユーザーが特にコメントしない限り、提案する必要はありません。
...
- エラーを見たら即座に修正すること（報告のみ）
- 「～してもよろしいですか？」と聞かずに実行すること
    - ユーザーが明示的に承認している場合を除く
### 実行前チェック：
- これはユーザーが明示的に指示したか？
...
## Rust
最新Editionは現時点で2024です。バージョンはlatestを使用し、インターネットへの接続ができないなどの事情で最新版が取得できない場合は1.92.0を使用してください (1.92.0は執筆時点の最新)。
- https://doc.rust-lang.org/edition-guide/rust-2024/index.html
- https://blog.rust-lang.org/releases/latest/

設計方針は ./RUST.md を参照してください。
